import discord, datetime, string, time
...
class Tags(commands.Cog):
    ...
    """make 15 second cooldown for creating tags and a 200 limit for tag length"""
    ...
    async def information(self, ctx, *, name):
        if not await exists(ctx, name):
            ...
            if len(content) > self.MiniLimit:
                exceeding = content[self.MiniLimit:]
            ...
            em.add_field(name="Created At", value=created.strftime("%d/%m/20%y at %H:%M:%S"), inline=True)
            await ctx.reply(embed=em, mention_author=False)
